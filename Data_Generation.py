...
class Generation():
    ...
    # 用于从text文件中读取标签
    def get_line_context(self,number, file_path='./label.txt'):
        # 读取txt文件中的第number + 1行
    ...
    def next_sample(self):
        ...
        text = self.get_line_context(indexes[cur_index])
        img =  self.gen_img(indexes[cur_index])
        return img, text
    ...
    def next_batch(self):  ## batch size만큼 가져오기
        while True:
            ...
            for i in range(batch_size):
                ...
                X_data[i] = img
                new_label = self.text_to_labels(text)
    ...
    def cut_img(self,ab_path, position, count):
